...
int main()
{
    ...
    while (1)
    {
        wsColor colors[NUM_PIXELS] = {
            HSBtoRGB(fmod(a, 360), 1.0, 0.1),
            HSBtoRGB(fmod(a + 90, 360), 1.0, 0.1),
            HSBtoRGB(fmod(a + 180, 360), 1.0, 0.1),
            HSBtoRGB(fmod(a + 270, 360), 1.0, 0.1)};

        uint8_t red[NUM_PIXELS] = {colors[0].r, colors[1].r, colors[2].r, colors[3].r};
        uint8_t green[NUM_PIXELS] = {colors[0].g, colors[1].g, colors[2].g, colors[3].g};
        uint8_t blue[NUM_PIXELS] = {colors[0].b, colors[1].b, colors[2].b, colors[3].b};

        for (int i = 0; i < NUM_PIXELS; i++)
        {
            // put_pixel(pio, sm, urgbw_u32(red[i], green[i], blue[i], 0));
            put_pixel(pio, sm, urgb_u32(red[i], green[i], blue[i]));
        }
        a = fmodf(a + (360.0f / (5.0f * 100.0f)), 360.0f);
        sleep_ms(1);
    }
